...
public class sceMpeg extends HLEModule {
    ...
    // ALVARO
    public static boolean a_videoPlaying = false;
    private Thread correctVideo;
    ...
    public int sceMpegGetAvcAu(@CheckArgument("checkMpegHandle") int mpeg, int streamUid, TPointer auAddr, @CanBeNull TPointer32 attrAddr) {
        if (this.correctVideo != null) {
            this.correctVideo.interrupt();
            this.correctVideo = null;
        }
        if (mpegRingbuffer != null) {
            mpegRingbuffer.read(mpegRingbufferAddr);
        }
        ...
        // ALVARO
        a_videoPlaying = !endOfVideoReached;
        // add timer for videos to stop (This function is called to read, which means if it is not called the video stops.)
        if (this.correctVideo == null) {
            this.correctVideo = new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        Thread.sleep(200);
                        a_videoPlaying = false;
                    } catch (InterruptedException e) {
                        
                    }
                }
            });
            this.correctVideo.start();
        }
        // ALVARO END
        return result;
    }
