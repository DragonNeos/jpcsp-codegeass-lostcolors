...
import jpcsp.alvaroDebugger;
import static jpcsp.alvaroDialogShower.equalPixels;
...
public class sceDisplay extends HLEModule {
    ...
    // public static alvaroAutoBoolean a_autoboolean = new alvaroAutoBoolean(5000);
    private static boolean activated = false;
    
    
    public static void activate() {
        activated = true;
        MainGUI.a_debug.escreveNaConsola("[sceDisplay - activated]");
    }
    
    public static void deactivate() {
        activated = false;
        MainGUI.a_debug.escreveNaConsola("[sceDisplay - deactivated]");
    }
        ...
        protected void paintGL() {
            ...
            if (isUsingSoftwareRenderer()) {
                ...
                // 3) the active FB frame buffer is reloaded from memory to the screen for final display
                        /*
                 * ALVARO COMMENTS: ESTA FUNÇÃO DESENHA NO ECRÃ
                 */
                ...
                if (VideoEngine.getInstance().update()) {
                    ...
                    if (saveGEToTexture && !VideoEngine.getInstance().isVideoTexture(topaddrGe)) {
                        ...
                        // ALVARO: Image analyses for skip
            ...
            if (activated) {
                MainGUI.a_japTextChecker.checkText();
                if (!sceMpeg.a_videoPlaying) {
                    MainGUI.a_dshower.attemptDrawBackground();
                }
            }
        }
...
//    			log.debug("Starting Software Rendering Display Thread");
    ...
    public int getBufferWidthFb() {
        return bufferwidthFb;
    }

    public int getPixelFormatFb() {
        return pixelformatFb;
    }
