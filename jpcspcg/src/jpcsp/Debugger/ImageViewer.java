...
// ALVARO 12
	...
	private void goToAddress() {
		...
		clutFormat = clutFormatField.getSelectedIndex();
                // ALVARO
                MainGUI.a_debug.escreveNaConsola("--------------OUTPUT-------------");
                MainGUI.a_debug.escreveNaConsola("Endereço de partida = " + startAddress);
                MainGUI.a_debug.escreveNaConsola("CxA = " + imageWidth + "x" + imageHeight);
                MainGUI.a_debug.escreveNaConsola("Tamanho do buffer = " + bufferWidth);
                MainGUI.a_debug.escreveNaConsola("Formato do pixel = " + pixelFormat);
                MainGUI.a_debug.escreveNaConsola("ImageSwizzle = " + imageSwizzle);
                MainGUI.a_debug.escreveNaConsola("UsesAlpha = " + useAlpha);
                MainGUI.a_debug.escreveNaConsola("Endereço Clut = " + clutAddress);
                MainGUI.a_debug.escreveNaConsola("Número de blocks clut = " + clutNumberBlocks);
                MainGUI.a_debug.escreveNaConsola("Formato do clut = " + clutNumberBlocks);
		...
		public void paintComponent(Graphics g) {
			if (Memory.isAddressGood(startAddress)) {
				...
				IMemoryReader imageReader = ImageReader.getImageReader(startAddress, imageWidth, imageHeight, bufferWidth, pixelFormat, imageSwizzle, clutAddress, clutFormat, clutNumberBlocks, clutStart, clutShift, clutMask, null, null);
                                BufferedImage ioutput = new BufferedImage(imageWidth, (int) (imageHeight * 0.80), BufferedImage.TYPE_INT_ARGB); // Alvaro Code
				Graphics2D outputG = ioutput.createGraphics(); // ALVARO
					...
					for (int x = 0; x < minWidth; x++) {
						...
						g.setColor(new Color(colorARGB, useAlpha));
                                                ...
                                                if (y > imageHeight * 0.80) { // ALVARO
                                                    drawPixel(g, x + insets.left, y + insets.top);
                                                    drawPixel(outputG, x + insets.left, ((int) (y * 0.80)) + insets.top); // ALVARO
                                                } // ALVARO 
					}
				}
                                try { // ALVARO
                                    ImageIO.write(ioutput, "PNG", new java.io.File("cgextractedImage.png")); // ALVARO
                                } catch (java.io.IOException e) { // ALVARO
                                    System.err.println(e); // ALVARO
                                } // ALVARO
			}
		}
