...
public class Controller {
    ...
    private static boolean SKIP_BUTTON_ENABLED = true;
    private static boolean R_BUTTON_ENABLED = true;
    private static boolean DISABLE_R_BUTTON_ENABLING = false;
    ...
    public void keyPressed(KeyEvent keyEvent) {
        //TODO tratar de quando for eventos
        //final KeyEvent thiskey = keyEvent;
        ...
        if (key == null || key == lastKey) {
            ...
            switch (key) {
                ...
                case TRIANGLE:  if (!MainGUI.isFrameRateLimited() || MainGUI.a_ep.isVisible()) { Buttons |= PSP_CTRL_TRIANGLE; } break;
                case SQUARE:    Buttons |= PSP_CTRL_SQUARE; break;
                case CIRCLE:    if (MainGUI.a_dshower != null) { MainGUI.a_dshower.skipTextAnimation(); } Buttons |= PSP_CTRL_CIRCLE; this.disableGUIHider(); break;
                case CROSS:     Buttons |= PSP_CTRL_CROSS; break;
                case L1:        Buttons |= PSP_CTRL_LTRIGGER; break;
                case R1:        if (R_BUTTON_ENABLED && !MainGUI.isFrameRateLimited()) { Buttons |= PSP_CTRL_RTRIGGER; } break;
    ...
    private void processControllerEvent(Component component, float value) {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("Controller Event on %s(%s): %f", component.getName(), id.getName(), value));
                ...
                if (id.getName().equals("1") && ENABLED) {
                    if (MainGUI.a_dshower != null) {
                        MainGUI.a_dshower.skipTextAnimation(); 
                    }
                } else if (id.getName().equals("7") && ENABLED) {
                    if (!R_BUTTON_ENABLED) {
                        return;
                    } else {
                        if (MainGUI.isFrameRateLimited()) {
                            if (!MainGUI.a_ep.isVisible()) {
                                return;
                            }
                        }
                    }
                } else if (id.getName().equals("9")) {
                    if (!SKIP_BUTTON_ENABLED) {
                        return;
                    }
                    
                } else if (id.getName().equals("0")) {
                    if (MainGUI.isFrameRateLimited()) {
                        return;
                    }
                } else if (!ENABLED) {
    ...
    public static void disableVideoSkipping() {
        SKIP_BUTTON_ENABLED = false;
    }
    
    public static void enableVideoSkipping() {
    ...
    public void stopAutoSkip() {
        //System.err.println(MainGUI.a_es.isAutoSkipping());
        if (MainGUI.a_es.isAutoSkipping()) {
            Buttons |= PSP_CTRL_LTRIGGER;
        }
    }
    
    public void disableGUIHider() {
        R_BUTTON_ENABLED = false;
    }
    
    public void enableGUIHider() {
        if (!DISABLE_R_BUTTON_ENABLING) {
            R_BUTTON_ENABLED = true;
        }
    }
    
    public void turnOnGUIHiderFunct() {
        DISABLE_R_BUTTON_ENABLING = false;
    }
    
    public void turnOffGUIHiderFunct() {
        DISABLE_R_BUTTON_ENABLING = true;
    }
