    	...
    	if (!initialized) {
    		try {
		        // Disable Xuggler's logging, since we do our own.
                        MainGUI.a_debug.escreveNaConsola("java.library.path");
    ...
    private boolean readAu(StreamState state, SceMpegAu au) {
    	...
    	if (state == null) {
    		...
    		state.getTimestamps(au);
                //MainGUI.a_debug.escreveNaConsola("READING MOVIE STATE: " + au.toString()); // ALVARO
                //MainGUI.a_debug.escreveNaConsola("READING MOVIE STATE: " + videoStreamState.toString()); // ALVARO
    	}
